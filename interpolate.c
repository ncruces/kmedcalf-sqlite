...
// SQLITE_INNOCUOUS Added for functions and aggregates but not added for virtual tables
...
/* The "interpolate" virtual table for SQLite3
   ...
   * Only INTEGER and REAL columns are supported.  Everything else
   ...
   To compile: gcc -std=c99 -O3 -g -fPIC -lm -shared interpolate.c -o libsqliteinterpolate.so
   ...
   Usage: CREATE VIRTUAL TABLE v USING
          interpolate(tab1, [tab2, ..., tabn][, key[, col1, ..., coln]][, period1 ..., periodn][, filter %][, subsampling /]);
   ...
   * avg_time(value, timestamp): Like the regular avg(), but it will
   ...
   * To get the moving average:
     ...
     SELECT avg_time(temp, ts) FROM v
     WHERE ts >= strftime("%s", "2012-01-31 06:00:00") AND ts <= strftime("%s", "2012-02-01 06:00:00");
   ...
   * To get the average daily low and high:
     ...
     SELECT avg_time(temp__min, ts), avg_time(temp__max, ts) FROM v WHERE ...;
   ...
   * To get the biggest difference between daily low and high:
   ...
   avg_time(dir, ts), avg_angle(dir__angle, ts) FROM ... will give
...
static void havg_angle_func(sqlite3_context *context, int argc, sqlite3_value **argv)
{
  ...
  count ? sqlite3_result_double(context, fmod(atan2(sumy, sumx) * 180 / M_PI + 360, 360)) : sqlite3_result_null(context);
}
...
static void hexagesimal_func(sqlite3_context *context, int argc, sqlite3_value **argv)
{
  ...
  if (min < 0) min = 0;
  if (min >= 60) min = 60 - DBL_MIN;
  if (sec < 0) sec = 0;
  if (sec >= 60) sec = 60 - DBL_MIN;

  char *s = sqlite3_mprintf("%%d%c%c %%02d\' %%0%d.%df\"", 0xc2, 0xb0, decimals + 3, decimals);
  char *buf = sqlite3_mprintf(s, (int)deg * sign, (int)min, sec);
  sqlite3_free(s);
...
static void avg_finalize(sqlite3_context *context)
{
  ...
  p && p->init && p->total > 0 ?
    sqlite3_result_double(context, p->sum / p->total) :
    sqlite3_result_null(context);
}
...
typedef struct {
  ...
  int *table_start;           // Their lowest timestamp
  int *table_end;             // Their highest timestamp
...
typedef struct {
  ...
  struct {
    ...
    double min, max;
    int sorted;
    int j[3]; } *last;        // Used for moving averages
...
static int find_primary_key(sqlite3 *db, const char *table, int *pk)
{
  ...
  *pk = -1;
  ...
  do {
    ...
    if (sqlite3_column_int64(stmt, 5) && !sqlite3_strnicmp((const char*)sqlite3_column_text(stmt, 2), "INTEGER", 7)) {
      *pk = j;
      break;
    }
    j++;
...
static int interpolate_connect(sqlite3 *db, void *pAux, int argc, const char *const* argv, sqlite3_vtab **ppVtab, char **pzErr)
{
  interpolate_vtab *v;
  ...
  *ppVtab = (sqlite3_vtab*)v;
  ...
  for (i = 3, k = 0; i < argc; i++) {
    ...
    case 'M': factor = 2592000; break;
    case 'y': factor = 31536000; break;
    default: factor = 1;
    }
  ...
  if (!first || i == argc) {
    ...
    if (sqlite3_strnicmp(pkname, v->timestamp, v->timestamp_len > len ? v->timestamp_len : len))
      return SQLITE_MISUSE;
      }
      sqlite3_finalize(stmt);
    }
  ...
  /* Copy description from source table.  We don'y need a union of the
     timestamps since we're only interested in the column information */
  char **bufs = sqlite3_malloc((2 + v->tablenum) * sizeof(char*));
  if (!bufs)
    return SQLITE_NOMEM;
  j = 0;
  bufs[j++] = sqlite3_mprintf("SELECT %s FROM (SELECT %s FROM %s LIMIT 1) ", v->selection, v->timestamp, v->tables[0]);
  ...
  for (i = 0; i < v->tablenum; i++) {
    /* For some reason it's much faster to select twice than ask for both in one query */
    char *buf = sqlite3_mprintf("SELECT (SELECT %s FROM %s LIMIT 1), (SELECT max(%s) FROM %s)",
                v->timestamp, v->tables[i], v->timestamp, v->tables[i]);
    //printf("%s: %s\n", v->table, buf);
  ...
  for (i = 0; i < cols; i++) {
    ...
    return SQLITE_NOMEM;
      ...
      sprintf(v->colnames[i + cols * 1 + j * cols * extras], "%s__avg%s", n, v->context_names[j]);
      sprintf(v->colnames[i + cols * 2 + j * cols * extras], "%s__min%s", n, v->context_names[j]);
      sprintf(v->colnames[i + cols * 3 + j * cols * extras], "%s__max%s", n, v->context_names[j]);
      sprintf(v->colnames[i + cols * 4 + j * cols * extras], "%s__angle%s", n, v->context_names[j]);
      sprintf(v->colnames[i + cols * 5 + j * cols * extras], "%s__angle_avg%s", n, v->context_names[j]);

      if (v->filter)
    sprintf(v->colnames[i + cols * 6 + j * cols * extras], "%s__avg_filtered%s", n, v->context_names[j]);
    }
  }
  ...
  for (i = 0; i < cols * (1 + extras * v->contexts_num); i++) {
    ...
    bufs[j++] = sqlite3_mprintf("%s %s%s%s",
                v->colnames[i],
                i < cols || (i / cols - 1) % extras == 1 ||
                (i / cols - 1) % extras == 2 ? (type ? type : "") : "REAL",
                i / cols == 0 ? "" : " HIDDEN",
...
static int interpolate_disconnect(sqlite3_vtab *vtab)
{
  ...
  for (i = 0; i < v->tablenum; i++)
    if (v->tables[i]) sqlite3_free(v->tables[i]);
  if (v->tables) sqlite3_free(v->tables);
  if (v->selection) sqlite3_free(v->selection);
  if (v->contexts) sqlite3_free(v->contexts);
  if (v->tablename_lens) sqlite3_free(v->tablename_lens);
  if (v->table_start) sqlite3_free(v->table_start);
  if (v->table_end) sqlite3_free(v->table_end);
  if (v->timestamp) sqlite3_free(v->timestamp);
  if (v->coltypes) sqlite3_free(v->coltypes);
  for (i = 0; i < v->cols; i++) {
    if (v->colnames[i + v->cols * 0]) sqlite3_free(v->colnames[i + v->cols * 0]);
    if (v->colnames[i + v->cols * 1]) sqlite3_free(v->colnames[i + v->cols * 1]);
    if (v->colnames[i + v->cols * 2]) sqlite3_free(v->colnames[i + v->cols * 2]);
    if (v->colnames[i + v->cols * 3]) sqlite3_free(v->colnames[i + v->cols * 3]);
    if (v->colnames[i + v->cols * 4]) sqlite3_free(v->colnames[i + v->cols * 4]);
    if (v->colnames[i + v->cols * 5]) sqlite3_free(v->colnames[i + v->cols * 5]);
    if (v->filter)
      if (v->colnames[i + v->cols * 6]) sqlite3_free(v->colnames[i + v->cols * 6]);
  }
  ...
  if (v->colnames) sqlite3_free(v->colnames);
  if (v->context_names) sqlite3_free(v->context_names);
...
static int interpolate_close(sqlite3_vtab_cursor *cur)
{
  ...
  for (i = 0; i < c->row_context; i++)
    if (c->context_values[i]) sqlite3_free(c->context_values[i]);

  if (c->last) sqlite3_free(c->last);
  if (c->context_values) sqlite3_free(c->context_values);
  if (c->timestamp_min) sqlite3_free(c->timestamp_min);
...
static int check_constraints(interpolate_cursor *c)
{
  ...
  for (i = 0; i < c->constraints_num; i++)
    ...
    case SQLITE_INDEX_CONSTRAINT_EQ: res &= c->timestamp == c->constraints[i].rval;
      break;
    case SQLITE_INDEX_CONSTRAINT_GT: res &= c->timestamp > c->constraints[i].rval;
      break;
    case SQLITE_INDEX_CONSTRAINT_LE: res &= c->timestamp <= c->constraints[i].rval;
      break;
    case SQLITE_INDEX_CONSTRAINT_LT: res &= c->timestamp < c->constraints[i].rval;
      break;
    case SQLITE_INDEX_CONSTRAINT_GE: res &= c->timestamp >= c->constraints[i].rval;
      break;
    }
  return res;
}
...
static int resize_buffer(interpolate_cursor *c)
{
  ...
  do {
    ...
    if (c->timestamp_min[i] == DBL_MAX && !isnan(c->context_values[j][i]))
        c->timestamp_min[i] = sqlite3_column_double(c->stmt, v->timestamp_idx);
      }
    } else
      if (rc == SQLITE_DONE)
    c->stmt_done = 1;
...
static int interpolate_next(sqlite3_vtab_cursor *cur)
{
  ...
  do {
    ...
    while (!c->stmt_done && c->row_context <= 60*60*24*31 && // More than one month at second resolution might be a runaway queries
       (((!isnan(c->context_values[c->context_end % c->row_context][v->timestamp_idx]) &&
         ...
         (!isnan(c->context_values[(c->context_end + 1) % c->row_context][v->timestamp_idx]) &&
          c->timestamp - c->context_values[(c->context_end + 1) % c->row_context][v->timestamp_idx] <= v->context)))) {
    ...
    for (i = 0; i < v->cols; i++) {
      c->context_values[c->context_end][i] =
        ...
        myNAN :
        sqlite3_column_double(c->stmt, i);
      if (c->timestamp_min[i] == DBL_MAX && !isnan(c->context_values[c->context_end][i]))
        c->timestamp_min[i] = sqlite3_column_double(c->stmt, v->timestamp_idx);
    }
      }
      if (rc == SQLITE_DONE)
...
static double calcmean(interpolate_cursor *c, double timestamp, double size, int k, int x)
{
  ...
  for (j = c->last[k].j[x] != -1 ? c->last[k].j[x] : (x == 2 ? c->context_index : (c->context_end + 1) % c->row_context);
       j != c->context_end;
       j = (j + 1) % c->row_context) {
  ...
  last_value =
    (last_value * (c->context_values[j][v->timestamp_idx] - timestamp - size / 2) +
  ...
  for (j = (j + c->row_context - 1) % c->row_context;
       j != c->context_end;
    ...
    if (timestamp - c->context_values[j][v->timestamp_idx] >= size / 2)
      break;
    valsum += (c->context_values[j][k % c->cols] + last_value) / 2 * (last_timestamp - c->context_values[j][v->timestamp_idx]);
    last_timestamp = c->context_values[j][v->timestamp_idx];
    last_value = c->context_values[j][k % c->cols];
  }
  ...
  double last =
    (c->context_values[j][k % c->cols] * (last_timestamp - timestamp + size / 2) +
...
static double calcmean_angle(interpolate_cursor *c, double timestamp, double size, int k, int x)
{
  ...
  if (last_value - this_value > 180)
    this_value += 360;
  double val = fmod((last_value * (c->context_values[j][v->timestamp_idx] - timestamp - size / 2) +
             this_value * (timestamp + size / 2 - last_timestamp)) /
            (c->context_values[j][v->timestamp_idx] - last_timestamp) + 360, 360);
  ...
  val = fmod((this_value * (last_timestamp - timestamp + size / 2) +
          last_value * (timestamp - c->context_values[j][v->timestamp_idx] - size / 2)) /
         (last_timestamp - c->context_values[j][v->timestamp_idx]) + 360, 360);
  ...
  return fmod(atan2(valsumy + yy * (last_timestamp - timestamp + size / 2),
            valsumx + xx * (last_timestamp - timestamp + size / 2)) * 180 / M_PI + 360, 360);
}
...
static double interpolate_minmax(interpolate_cursor *c, double timestamp, double size, int k, int x, int returnmin, int *sorted, double *res2)
{
  double last_value = myNAN;
  double last_timestamp = myNAN;
  int last_j = -1;
  ...
  s &= j != c->context_end && !isnan(c->context_values[(j + 1) % c->row_context][k % c->cols]) &&
    (returnmin ? le(last_value, c->context_values[(j + 1) % c->row_context][k % c->cols]) : ge(last_value, c->context_values[(j + 1) % c->row_context][k % c->cols]));
  /* Retry with full scan if this didn't work */
  if (isnan(last_value) && c->last[k].j[x] != -1) {
    c->last[k].j[x] = -1;
  ...
  for (j = (j + c->row_context - 1) % c->row_context;
    ...
    if (timestamp - c->context_values[j][v->timestamp_idx] >= size / 2)
      break;
    s &= returnmin ? ge(last_value, c->context_values[j][k % c->cols]) : le(last_value, c->context_values[j][k % c->cols]);
  ...
  s &= j != c->context_end && (j + c->row_context - 1) % c->row_context != c->context_end &&
    (returnmin ? ge(last_value, c->context_values[(j + c->row_context - 1) % c->row_context][k % c->cols]) : le(last_value, c->context_values[(j + c->row_context - 1) % c->row_context][k % c->cols]));
  if (min > last)
    min = last;
  if (max < last)
    max = last;
...
static int interpolate_column(sqlite3_vtab_cursor *cur, sqlite3_context *ctx, int i)
{
  ...
  if (ctype != COL_BASE && ctype != COL_ANGLE) {
    ...
    c->timestamp + v->contexts[ccontext] / 2 > c->context_values[c->context_end][v->timestamp_idx]) {
    ...
    for (j = c->context_index;
         j != c->context_end &&
         c->timestamp - c->context_values[j][v->timestamp_idx] <= v->contexts[ccontext] / 2;
         j = (j + c->row_context - 1) % c->row_context)
      if (!isnan(c->context_values[j][k]))
        f[vals++] = c->context_values[j][k];
    for (j = c->context_index;
         j != (c->context_end + 1) % c->row_context &&
         c->context_values[j][v->timestamp_idx] - c->timestamp <= v->contexts[ccontext] / 2;
         j = (j + 1) % c->row_context)
      if (!isnan(c->context_values[j][k]))
        f[vals++] = c->context_values[j][k];
    ...
    sqlite3_free(f);
      ...
      if (ctype == COL_AVG && !isnan(c->last[q].v) && c->timestamp - c->last[q].t < v->contexts[ccontext] / 4)
    /* Not for __angle, accuracy losses would accumulate */
    /* Nor for __avg_filtered, since the filter needs to be rerun */
    val = c->last[q].v +
      mean(c, (c->last[q].t + c->timestamp) / 2 + v->contexts[ccontext] / 2, c->timestamp - c->last[q].t, q, 0) -
      mean(c, (c->last[q].t + c->timestamp) / 2 - v->contexts[ccontext] / 2, c->timestamp - c->last[q].t, q, 1);
      else // Full scan
    ...
    sqlite3_result_double(ctx, round(precision * (val / (ctype == COL_AVG || ctype == COL_AVG_FILTERED ? v->contexts[ccontext] : 1))) / precision);
      return SQLITE_OK;
    } else {
      /* Min or max */
      ...
      double prev = interpolate_minmax(c, (c->last[q].t + c->timestamp) / 2 - v->contexts[ccontext] / 2, c->timestamp - c->last[q].t, q, 1, returnmin, &sorted_prev, &res2);
      double next = interpolate_minmax(c, (c->last[q].t + c->timestamp) / 2 + v->contexts[ccontext] / 2, c->timestamp - c->last[q].t, q, 0, returnmin, &sorted_next, 0);
      if (!isnan(last) && c->timestamp - c->last[q].t < v->contexts[ccontext] / 4 &&
      (!eq(last, prev) ||
       (sorted_prev && sorted_next && c->last[q].sorted && (returnmin ? le(prev, next) : ge(prev, next))))) {
    /* No, reuse last result */
    val = sorted_prev && sorted_next && c->last[q].sorted && (returnmin ? le(prev, next) : ge(prev, next)) ? res2 : (returnmin ? (next < last ? next : last) : (next > last ? next : last));
    sorted_next &= sorted_prev && c->last[q].sorted;
      } else // Yes, full scan
    val = interpolate_minmax(c, c->timestamp, v->contexts[ccontext], q, 2, returnmin, &sorted_next, 0);
    ...
    sqlite3_result_double(ctx, round(precision * val) / precision);
      }
      return SQLITE_OK;
    }
  ...
  if (c->timestamp == last_timestamp) {
    if (v->coltypes[i] == SQLITE_INTEGER)
      sqlite3_result_int64(ctx, last_value);
    else if (v->coltypes[i] == SQLITE_FLOAT)
      sqlite3_result_double(ctx, round(precision * last_value) / precision);
    return SQLITE_OK;
  } else
      ...
      if (dir) {
        if (this_value - last_value > 180)
          last_value += 360;
            if (last_value - this_value > 180)
          this_value += 360;
        val = fmod((this_value * (c->timestamp - last_timestamp) +
            last_value * (c->context_values[j][v->timestamp_idx] - c->timestamp)) /
               (c->context_values[j][v->timestamp_idx] - last_timestamp), 360);
      } else
        val =
...
static int interpolate_filter(sqlite3_vtab_cursor *cur, int idxNum, const char *idxStr, int argc, sqlite3_value **argv)
{
  interpolate_cursor *c = (interpolate_cursor*)cur;
  const interpolate_vtab *v = (interpolate_vtab*)c->cur.pVtab;
  int rc, i, j = 0, l;
  ...
  for (i = j = 0; i < argc; i++) {
    ...
    case '>':
      ...
      c->constraints[i].op = idxStr[1] == '=' ?
    SQLITE_INDEX_CONSTRAINT_GE : SQLITE_INDEX_CONSTRAINT_GT;
      if (idxStr[1]) eq_ts[eq++] = c->constraints[j].rval;
      constraints[j].op = idxStr[1] == '=' ?
    SQLITE_INDEX_CONSTRAINT_GE : SQLITE_INDEX_CONSTRAINT_GT;
  ...
  if (eq) {
    c->temptable = rand() | 1;
    char *buf = sqlite3_mprintf("CREATE TEMP TABLE temp.t%d ( %s %s )",
                c->temptable, v->timestamp,
    ...
    buf = sqlite3_mprintf("INSERT INTO temp.t%d VALUES ( %ld )",
                  c->temptable, (sqlite_int64)eq_ts[i]);
      else
    buf = sqlite3_mprintf("INSERT INTO temp.t%d VALUES ( %f )",
                  c->temptable, eq_ts[i]);
      rc = sqlite3_exec(v->db, buf, NULL, NULL, NULL);
      if (rc != SQLITE_OK)
    return rc;
  ...
  for (i = 0; i < argc; i++) {
    if (c->constraints[i].rval > start &&
    (c->constraints[i].op == SQLITE_INDEX_CONSTRAINT_GE || c->constraints[i].op == SQLITE_INDEX_CONSTRAINT_GT ||
     c->constraints[i].op == SQLITE_INDEX_CONSTRAINT_EQ))
      start = c->constraints[i].rval;
  }

  char **bufs = sqlite3_malloc((3 + !!eq +  v->tablenum) * sizeof(char*));
  if (!bufs)
    return SQLITE_NOMEM;
  j = 0;

  bufs[j++] = sqlite3_mprintf("SELECT count(%s) FROM (\n", v->timestamp);
  if (eq)
    bufs[j++] = sqlite3_mprintf("  SELECT %s FROM temp.t%d UNION\n",
                v->timestamp, c->temptable);
  for (i = 0; i < v->tablenum; i++) {
    bufs[j++] = sqlite3_mprintf("  SELECT %s FROM %s WHERE %s >= %f AND %s <= %f %s",
                v->timestamp, v->tables[i], v->timestamp, start - v->context, v->timestamp, start + v->context,
                i == v->tablenum - 1 ? "" : "UNION\n");
  }
  if (v->subsample > 1)
    bufs[j++] = sqlite3_mprintf("GROUP BY %s / %d ", v->timestamp, v->subsample);
  bufs[j++] = sqlite3_mprintf("ORDER BY %s)\n", v->timestamp, v->subsample, v->timestamp);
  char *stmtbuf;
  rc = concat_strings(bufs, j, &stmtbuf);
  if (rc != SQLITE_OK)
    return rc;
  ...
  /* Open the full range */
  bufs = sqlite3_malloc((7 + !!eq + 5 * v->tablenum) * sizeof(char*));
  ...
  for (k = 0; k < cn; k++) {
    ...
    case SQLITE_INDEX_CONSTRAINT_MATCH: op = "="; break;
    default: op = "?"; break;
    }
    bufs2[j++] = sqlite3_mprintf("%s%s %s %f%s",
                k ? "" : "WHERE ",
                v->timestamp, op, constraints[k].rval,
                k != cn - 1 ? " AND " : "");
  }
  ...
  if (k && k != i - 5 && !*where) {
...
#if 1 // Hopefully, query is only asking for max(unix_time), so we don't need all rows
    ...
    if (eq)
      bufs[j++] = sqlite3_mprintf("  SELECT %s FROM temp.t%d UNION\n",
                  v->timestamp, c->temptable);
    for (i = 0; i < v->tablenum; i++) {
      bufs[j++] = sqlite3_mprintf("  SELECT %s FROM %s %s",
    ...
    return SQLITE_NOMEM;
      ...
      if (rc != SQLITE_OK)
    return rc;
      bufs[j++] = sqlite3_mprintf("LEFT JOIN (SELECT %s FROM %s %s",
                buf, v->tables[i], where);
      if (v->subsample > 1)
    bufs[j++] = sqlite3_mprintf(" GROUP BY %s / %d", v->timestamp, v->subsample);
      bufs[j++] = sqlite3_mprintf(") USING(%s)\n", v->timestamp);
      sqlite3_free(buf);
    }
  }
...
static int interpolate_bestindex(sqlite3_vtab *tab, sqlite3_index_info *info)
{
  ...
  if (info->nConstraint) {
    ...
    continue;
      ...
      bufs[j++] = sqlite3_mprintf("%s %s %%%s", v->colnames[info->aConstraint[i].iColumn], op,
                  i == constraints - 1 ? "" : " AND ");
  ...
  info->orderByConsumed = !!info->nOrderBy &&
    (info->aOrderBy[0].iColumn == -1 || info->aOrderBy[0].iColumn == v->timestamp_idx);

  if (info->orderByConsumed)
    for (i = 0; i < info->nOrderBy; i++) {
      const char *col = info->aOrderBy[i].iColumn == -1 ? "rowid" : v->colnames[info->aOrderBy[i].iColumn];
      bufs[j++] = sqlite3_mprintf("%s%s%s", col,
                  info->aOrderBy[i].desc ? " DESC" : "", i + 1 == info->nOrderBy ? "" : ", ");
    }
  else
...
int sqlite3_interpolate_init(sqlite3 *db, char **pzErrMsg, const sqlite3_api_routines *pApi)
{
  ...
  static const struct {
    ...
    { "avg_time",         2, avg_step,       avg_finalize },
  ...
  for (i = 0; i < sizeof(functions) / sizeof(*functions); i++)
    sqlite3_create_function(db, functions[i].zFunctionName, functions[i].nArg, SQLITE_UTF8|SQLITE_DETERMINISTIC|SQLITE_INNOCUOUS,
                db, functions[i].xFunc, 0, 0);

  for (i = 0; i < sizeof(aggregates) / sizeof(*aggregates); i++)
    sqlite3_create_function(db, aggregates[i].zFunctionName, aggregates[i].nArg, SQLITE_UTF8|SQLITE_DETERMINISTIC|SQLITE_INNOCUOUS,
